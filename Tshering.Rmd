...
```{r}
rf_workflow <- rf_workflow %>% finalize_workflow(best_param_rf)
```
...
```{r}
svm_final <- svm_wflow %>% finalize_workflow(best_param_svm)
...
test_performance_svm <- svm_fit %>% collect_metrics()
test_performance_svm
```
...
```{r}
best_param_svmbo <- svm_bo %>%
              select_best(metric = "accuracy")
best_param_svmbo
```

```{r}
svm_bo_final <- svm_bo %>% finalize_workflow(best_param_svmbo)

svm_bo_fit <- svm_final %>%
  last_fit(Master_Split)

svm_bo_fit

test_performance_svmbo <- svm_bo_fit %>% collect_metrics()
test_performance_svmbo